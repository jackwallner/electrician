...
struct FlashcardDrillView: View {
    ...
    @AppStorage("electrician.hasSwipedDeck") private var hasSwipedDeck = false
...
struct FlipCardFace: View {
    ...
    private var front: some View {
        ElectricianCardFace(accent: accent, eyebrow: "ELECTRICIAN") {
            VStack(spacing: 18) {
                Spacer(minLength: 0)
    ...
    private var back: some View {
        ElectricianCardFace(accent: accent, eyebrow: "THE CALL") {
            VStack(spacing: 14) {
                if let verdict {
