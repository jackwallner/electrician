...
struct QuickSessionView: View {
    ...
    init(codeMinute challenge: CodeMinuteChallenge, onClose: (() -> Void)? = nil) {
        _items = State(initialValue: challenge.items)
        purpose = .codeMinute(challenge)
        self.onClose = onClose
    }
    ...
    @State private var minuteResult: CodeMinuteResult?
    ...
    private var completion: some View {
        if case .codeMinute = purpose, let minuteResult {
            CodeMinuteResultView(result: minuteResult, recordsCompletion: true, onDone: onClose)
        } else {
