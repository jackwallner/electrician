...
struct CodeMinuteView: View {
    @StateObject private var store = CodeMinuteStore.shared

    private var today: Date { Date() }
    private var challenge: CodeMinuteChallenge { CodeMinuteContent.challenge(for: today) }
    private var todayResult: CodeMinuteResult? { store.result(for: today) }
    ...
    private var hero: some View {
        VStack(spacing: 14) {
            ...
            if let todayResult {
                NavigationLink {
                    CodeMinuteResultView(result: todayResult)
                } label: {
    ...
    private func archiveRow(_ date: Date) -> some View {
        ...
        NavigationLink {
            if let result {
                CodeMinuteResultView(result: result)
            } else {
                QuickSessionView(codeMinute: CodeMinuteContent.challenge(for: date))
            }
        } label: {
            HStack(spacing: 12) {
...
struct CodeMinuteResultView: View {
    let result: CodeMinuteResult
    ...
    private func recordCompletionIfNeeded() {
        ...
        progress.recordSession(drillID: CodeMinuteContent.drill.id)
