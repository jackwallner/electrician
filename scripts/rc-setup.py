#!/usr/bin/env python3
"""Wire the Electrician App Store products into RevenueCat.
...
2. **The entitlement is `electrician_pro`.** `SubscriptionService` now checks
...
not stored in this repo; it lives in ~/.electrician_credentials. Pass it in the
environment.
"""
...
ENTITLEMENT_KEY = "electrician_pro"
ENTITLEMENT_NAME = "Electrician Pro"

# (store identifier, display name, RC product type, offering package key)
PRODUCTS = (
    ("com.jackwallner.electrician.monthly", "Monthly", "subscription", "$rc_monthly"),
    ("com.jackwallner.electrician.yearly", "Yearly", "subscription", "$rc_annual"),
    ("com.jackwallner.electrician.lifetime", "Lifetime", "one_time", "$rc_lifetime"),
)
...
def request(method: str, path: str, body: dict | None = None) -> dict:
    ...
    if not key:
        raise SystemExit(
            ...
            "       It lives in ~/.electrician_credentials as RC_SECRET_KEY."
        )
...
def main() -> None:
    ...
    project = (
        projects[0]
        if len(projects) == 1
        else next(p for p in projects if p["name"].lower() == "electrician")
    )
