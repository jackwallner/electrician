...
struct CodeMinuteResult: Codable, Identifiable, Sendable {
    let dayKey: String
    ...
    func correct(in category: CodeMinuteCategory) -> Int {
        correctByCategory[category.rawValue, default: 0]
    }

    func total(in category: CodeMinuteCategory) -> Int {
...
@MainActor
final class CodeMinuteStore: ObservableObject {
    static let shared = CodeMinuteStore()

    @Published private(set) var results: [String: CodeMinuteResult]

    private let defaults: UserDefaults
    private static let resultsKey = "codeMinute.results"

    init(defaults: UserDefaults = .standard) {
        self.defaults = defaults
        if let data = defaults.data(forKey: Self.resultsKey),
           let decoded = try? JSONDecoder().decode([String: CodeMinuteResult].self, from: data) {
            results = decoded
        } else {
            results = [:]
        }
